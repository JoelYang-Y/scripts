...
check_exit_ips() {
    ...
    # Google
    local google_ip=$(curl $CURL_OPT -4 https://services.google.com/fh/files/misc/checkip.html 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+' | head -1)
    print_status "Google 出口" "$google_ip" "$BLUE"
    
    # PayPal
    local paypal_ip=$(curl $CURL_OPT -4 https://www.paypal.com/cgi-bin/webscr?cmd=_display-ip 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+' | head -1)
