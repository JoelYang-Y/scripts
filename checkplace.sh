...
1)export NEWT_COLORS='
              ...
              entry=,green
            '
cmd="bash <(curl -Ls https://joel.jhsweetheart.com/ip.check.place)"
while true;do
if [[ -n $opti1 ]];then
msgi1="${smenu[set]}"
else
...
if [[ $IPV4check -eq 1 && $IPV6check -eq 1 && $IPV4work -eq 1 && $IPV6work -eq 1 ]]&&[[ $cmd == *"joel.jhsweetheart.com/ip.check.place"* || $cmd == *"Net.Check.Place"* ]];then
